...
//import {MdGridListModule,MdInputModule,MdDatepickerModule,MdSelectModule,MdAutocompleteModule,MdButtonModule} from '@angular/material';
...
@Component({
  selector: 'record',
  templateUrl: './../../Views/Record/record.component.html',
  styles: [`
    .record-card{
		width:80%;
		margin-left: 10%;
  ...
  _updateRecord() {
    if (this.title == "Purchase") {
      ...
      if (this.billNo.valid && this.billDate.valid && this.CustomerName.valid) {
        ...
        if (!flag) {
          let billDate = new Date(this.billDate.value);
          this.salesHelper.CompareItems(this);
  ...
  _compareItems() {
    ...
    this.selectedItems.forEach(function (item, index) {
      ...
      scope.OldSelectedItems.forEach(function (oldItem: any) {
        ...
        if (item.Pid == oldItem.Pid && item.BatchNo == oldItem.BatchNo && item.qty != oldItem.qty) {
          ...
          if (item.qty < oldItem.qty && (oldItem.stock - (oldItem.qty - item.qty)) < 0) {
            let msg = "Cannot reduct quantity of " + item.Pname + ": Batch: " + item.BatchNo + ". Stock too low ";
            StockException = { message: msg };
          } else {
            oldItem.stock = item.qty < oldItem.qty ? (oldItem.stock - (oldItem.qty - item.qty)) : (oldItem.stock + (item.qty - oldItem.qty));
            oldItem.qty = item.qty;
            scope.UpdateItems_u.push(oldItem);
          }

        } else if (item.Pid == oldItem.Pid && item.BatchNo == oldItem.BatchNo && item.qty == oldItem.qty)
    ...
    if (!StockException) {
      this.OldSelectedItems.forEach(function (item) {
        if (!item.noChange && ((item.stock - item.qty) >= 0))
          scope.RemoveItems_u.push(item);
        else if (!item.noChange && ((item.stock - item.qty) < 0)) {
    ...
    if (this.title == "Purchase") {
      ...
      row.Batches.forEach(function (item: any) {
        ...
        if (row.qty <= row.stock && item.show == true && temp_qty > 0) {
          ...
          item.batchValue = item.mrp * item.batchQty;
